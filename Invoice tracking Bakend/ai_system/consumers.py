...
class AIInsightsConsumer(BaseAuthenticatedConsumer):
    """WebSocket consumer for real-time AI insights"""
    
    async def connect(self):
        await super().connect()
        
        # Join AI insights group
        await self.channel_layer.group_add(
            "ai_insights",
            self.channel_name
        )
    ...
    async def prediction_updated(self, event):
